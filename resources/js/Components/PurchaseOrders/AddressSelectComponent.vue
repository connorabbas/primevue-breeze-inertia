...
<template>
    <div class="space-y-4">
        <div v-for="(addresses, type) in formattedAddresses" :key="type" class="p-4 border rounded-lg">
            <div class="font-medium text-lg mb-2 capitalize">
                {{ type.replace(/([A-Z])/g, ' $1').trim() }} Address
                <span v-if="settings.requireShippingAddress && type === 'shipTo'" class="text-red-500">*</span>
            </div>

            <div v-if="addresses.length === 0" class="text-gray-500 italic">
